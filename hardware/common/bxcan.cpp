...
void hardware_can_Init(void)
{
	...
	cur_bitrate = settings.uavcan.bitrate;

	// if successful SetBitrate will start the can interfaces.
	while(!hardware::can::SetBitrate(cur_bitrate))
	{
 ...
 * @return				True on success.
 */
bool hardware::can::SetBitrate(const std::uint32_t bitrate)
{
	bool result = false;
	...
	if(			bitrate >= 125000
			&& 	bitrate <= 1000000)
	{
		result = hardware_can_ComputeTimings(STM32_PCLK1, bitrate, &timings);

		if(result)
		{
			cancfg.btr = 	  CAN_BTR_SJW(timings.max_resync_jump_width)
							| CAN_BTR_TS1(timings.bit_segment_1)
							| CAN_BTR_TS2(timings.bit_segment_2)
							| CAN_BTR_BRP(timings.bit_rate_prescaler);
